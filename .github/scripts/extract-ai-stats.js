...
function renderSummarySection(summary, heading) {
  const lines = [];
...
function generatePRComment(summary) {
  ...
  lines.push(...renderSummarySection(summary, "### This Change"));
  lines.push(
...
function generateCombinedReport(pushSummary, overallSummary) {
  const lines = [];
  lines.push("## 🤖 AI Authorship Report\n");

  // This push section
  lines.push(...renderSummarySection(pushSummary, "### 📌 This Push"));

  // Separator
  lines.push("---\n");

  // Overall repo section
  lines.push(...renderSummarySection(overallSummary, "### 📊 Overall Repository"));
...
const includeOverall = args.includes("--overall");
...
if (format === "markdown" || format === "md") {
  if (includeOverall && range) {
    // Build overall summary from all commits in the repo
    const allCommits = extractCommits(null);
    const allStats = extractStats(allCommits);
    const overallSummary = buildSummary(allStats);
    output = generateCombinedReport(summary, overallSummary);
  } else {
